...
const WAVE2_CONCEPTS = () => CONCEPTS.filter((c) => /^VMA-3[3-9]$|^VMA-4[0-6]$/.test(c.number || ''));

function priorityChip(p) {
  if (!p) return '';
  return `<span class="chip" style="font-weight:700">${esc(p)}</span>`;
}

function wave2ConceptCard(c) {
  const ratios = (c.staticFormats || []).join(' · ');
  const ratioDetails = c.ratioCompositions
    ? Object.entries(c.ratioCompositions)
        .map(
          ([r, note]) =>
            `<details class="block"><summary>${esc(r)} layout</summary><p style="font-size:0.82rem;margin:0.35rem 0">${esc(note)}</p></details>`,
        )
        .join('')
    : '';
  const prod =
    c.remotionNotes || c.capcutNotes
      ? `<p class="note"><b>Motion:</b> ${esc(c.remotionNotes || '')} ${esc(c.capcutNotes || '')}</p>`
      : '';
  return `<article class="soft-card" id="${esc(c.number)}">
  <div class="master-card__meta">${esc(c.number)} · ${statusBadge(c.testingTag || '—')} ${priorityChip(c.recommendedPriority)} · ${esc(c.recommendedAudience || '')}</div>
  <h3>${esc(c.name)}</h3>
  <p><b>${esc(c.headline)}</b></p>
  <p>${esc(c.supportingLine)}</p>
  <p>${(c.benefits || []).map((b) => `<span class="chip">${esc(b)}</span>`).join(' ')}</p>
  <p><span class="chip">${esc(c.colorFamilyId)}</span> <span class="chip">${esc(ratios)}</span> ${statusBadge(c.productionStatus)}</p>
  <p class="note"><b>Hypothesis:</b> ${esc(c.sourceHypothesis || '—')}</p>
  <p class="note">${esc(c.visualDirection || '')}</p>
  ${ratioDetails}
  ${prod}
</article>`;
}
...
function renderIdeas() {
  ...
  const body = `
    ...
    <section id="wave-2-review">
      <h2>Wave 2 — Controlled Testing (${esc(TESTING_WAVE_2.mix)})</h2>
      <div class="callout-rule">DO NOT LAUNCH ALL ${WAVE2_CONCEPTS().length} CONCEPTS AT ONCE. First live batch: ${TESTING_WAVE_2.firstLaunchBatch.static.join(', ')} + videos ${TESTING_WAVE_2.firstLaunchBatch.video.join(', ')}.</div>
      <p class="lede">${esc(TESTING_WAVE_2.firstLaunchBatch.note)}</p>
      <h3>P1 — produce &amp; test first</h3>
      <div class="queue-grid">${WAVE2_CONCEPTS()
        .filter((c) => c.recommendedPriority === 'P1')
        .map((c) => wave2ConceptCard(c))
        .join('')}</div>
      <h3>P2 — second wave</h3>
      <div class="queue-grid">${WAVE2_CONCEPTS()
        .filter((c) => c.recommendedPriority === 'P2')
        .map((c) => wave2ConceptCard(c))
        .join('')}</div>
      <h3>P3 — hold for later</h3>
      <div class="queue-grid">${WAVE2_CONCEPTS()
        .filter((c) => c.recommendedPriority === 'P3')
        .map((c) => wave2ConceptCard(c))
        .join('')}</div>
      <p class="note">Full prompts on <a href="/vma-chatgpt.html#wave-2-prompts">Prompts &amp; Copy</a> · Animations on <a href="/vma-video.html#wave-2-video">Animated Video</a>.</p>
    </section>
...
function renderVideo() {
  ...
  <p>${esc(p.duration)} · ${esc(p.colorFamily)} ${p.testingTag ? `· ${statusBadge(p.testingTag)} ${priorityChip(p.recommendedPriority)}` : ''}</p>
  ${copyBlock(p.prompt)}
</div>`,
  ).join('');

  const wave2VideoNums = new Set(['VMA-V22', 'VMA-V23', 'VMA-V24', 'VMA-V25', 'VMA-V26', 'VMA-V27', 'VMA-V28']);
  const wave2Videos = VIDEO_PROMPTS.filter((p) => wave2VideoNums.has(p.videoNumber));

  const wave2VideoCards = wave2Videos
    .map(
      (p) => `<div class="soft-card">
  <h3>${esc(p.videoNumber || p.id)} · ${esc(p.title)}</h3>
  <p>${statusBadge(p.testingTag || 'Scale')} ${priorityChip(p.recommendedPriority)}</p>
  ${copyBlock(p.prompt)}
</div>`,
    )
    .join('');
  ...
  const body = `
    ...
    <section id="wave-2-video">
      <h2>Wave 2 Animations (VMA-V22..V28)</h2>
      <div class="callout-rule">Animate static winners AFTER static QA. Export 9:16 · 4:5 · 1:1 per spot — no 1.91:1 video required.</div>
      <p class="lede">First motion batch: ${TESTING_WAVE_2.firstLaunchBatch.video.join(', ')}</p>
      <div class="queue-grid">${wave2VideoCards}</div>
    </section>
...
function renderChatgpt() {
  ...
  const imagePrompts = CHATGPT_PROMPTS.map(
    (p) => `<div class="soft-card" id="${esc(p.conceptNumber || p.id)}">
  <h3>${esc(p.id)} · ${esc(p.title)}</h3>
  <p>${esc(p.conceptNumber)} · ${esc(p.colorFamily)} · ${(p.ratios || []).join(', ')}</p>
  ${p.testingTag ? `<p>${statusBadge(p.testingTag)} ${priorityChip(p.recommendedPriority)}</p>` : ''}
  ${p.sourceHypothesis ? `<p class="note"><b>Hypothesis:</b> ${esc(p.sourceHypothesis)}</p>` : ''}
  ${copyBlock(p.prompt)}
  ${p.remotionNotes ? `<p class="note"><b>Remotion:</b> ${esc(p.remotionNotes)}</p>` : ''}
  ${p.capcutNotes ? `<p class="note"><b>CapCut:</b> ${esc(p.capcutNotes)}</p>` : ''}
</div>`,
  ).join('');

  const wave2PromptIds = new Set(WAVE2_CONCEPTS().map((c) => c.number));
  const wave2ImagePrompts = CHATGPT_PROMPTS.filter((p) => wave2PromptIds.has(p.conceptNumber))
    .map(
      (p) => `<div class="soft-card">
  <h3>${esc(p.id)} · ${esc(p.title)}</h3>
  <p>${statusBadge(p.testingTag)} ${priorityChip(p.recommendedPriority)}</p>
  ${copyBlock(p.prompt)}
</div>`,
    )
    .join('');
  ...
  const videoPrompts = VIDEO_PROMPTS.map(
    (p) => `<div class="soft-card">
  <h3>${esc(p.id)} · ${esc(p.title)}</h3>
  <p>${esc(p.duration)} · ${p.testingTag ? `${statusBadge(p.testingTag)} ${priorityChip(p.recommendedPriority)}` : ''}</p>
  ${copyBlock(p.prompt)}
  ${p.remotionNotes ? `<p class="note"><b>Remotion:</b> ${esc(p.remotionNotes)}</p>` : ''}
  ${p.capcutNotes ? `<p class="note"><b>CapCut:</b> ${esc(p.capcutNotes)}</p>` : ''}
</div>`,
  ...
  const body = `
    ...
    <section data-tabs id="banks">
      <div class="tabs">
        ...
        <button type="button" data-tab="wave2">Wave 2 prompts</button>
      ...
      <div class="tab-panels">
        ...
        <div class="tab-panel" data-panel="wave2" id="wave-2-prompts">
          <h3>Wave 2 image prompts only (VMA-33..46)</h3>
          <p class="note">Purpose-built per-ratio plates — see each concept card for 1:1 · 4:5 · 9:16 · 1.91:1 layout notes on <a href="/ideas.html#wave-2-review">New Ad Ideas</a>.</p>
          <div class="prompt-grid">${wave2ImagePrompts}</div>
        </div>
...
function renderHandoff() {
  ...
  const body = `
    ...
    <section id="wave-2-handoff">
      <h2>Wave 2 — Controlled Testing Handoff</h2>
      <div class="job-box">
        <p><b>${esc(TESTING_WAVE_2.label)}</b> — ${esc(TESTING_WAVE_2.mix)}</p>
        <p><b>First launch (static):</b> ${TESTING_WAVE_2.firstLaunchBatch.static.map((n) => `<code>${esc(n)}</code>`).join(' · ')}</p>
        <p><b>First launch (video, after static QA):</b> ${TESTING_WAVE_2.firstLaunchBatch.video.map((n) => `<code>${esc(n)}</code>`).join(' · ')}</p>
        <p>${esc(TESTING_WAVE_2.firstLaunchBatch.note)}</p>
        <p class="note">Review all concepts on <a href="/ideas.html#wave-2-review">New Ad Ideas → Wave 2</a>. Prompts: <a href="/vma-chatgpt.html#wave-2-prompts">Wave 2 prompts tab</a>.</p>
        <p><a class="dl" href="/wave1-launch.html"><strong>→ Wave 1 ACTIVE launch board</strong></a> (5 statics — upload package ready)</p>
      </div>
      <h3>Claim-gated / On Hold</h3>
      <ul class="clean">${TESTING_WAVE_2.claimGated.map((g) => `<li><b>${esc(g.concept)}</b> — ${esc(g.claim)}</li>`).join('')}</ul>
    </section>
